...
Root Entry/
  ...
  CIS/
    VariantStore/
      BOM/
        {Variant}/{Variant}          # exact groups selected by one BOM variant
      Groups/
...
CIS/
  VariantStore/
    ...
    BOM/
      BOMDataStream
      {Variant}/
        {Variant}                          # exact group membership for this BOM variant
...
| `CIS/VariantStore/BOM/{v}/{v}` | Group names selected by one BOM variant | Fully parsed |
| `CIS/VariantStore/VariantNames` | Variant, group, and helper names | Parsed for diagnostics |
...
### 11.2 BOM variant group membership

The stream `CIS/VariantStore/BOM/{variant}/{variant}` defines the groups selected
by one actual BOM variant:

```
    uint32    payload_length
    latin1    group_count 0xF9 group_name 0xF9 group_name ...
```

The first field is the decimal number of group names that follow. `Common`
normally appears without a corresponding group stream because it contributes no
variant-specific stuffing override. For example, the `Standard` variant in
`LAUNCHXL-CC1310` lists `Common`, `DNM`, `DebuggerIF`, `Peripherals`, `RF`, and
`XDS`; resolving only those groups produces the assembly represented by its BOM.

This stream is also the authoritative list of actual queryable variants. The
nearby `VariantNames` stream repeats BOM names, group names, and `bom-...` helper
names, so treating every name in it as an assembly would invent selectors that
do not exist.

### 11.3 Occurrence ids and the join to a refdes
...
### 11.5 Measured against a board's own BOM
