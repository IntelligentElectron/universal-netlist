...
describe("isUniversalFile", () => {
  it("accepts .json in any case and nothing else", () => {
    expect(isUniversalFile("a/b.json")).toBe(true);
    expect(isUniversalFile("a/B.JSON")).toBe(true);
    expect(isUniversalFile("a/b.kicad_pro")).toBe(false);
    expect(isUniversalFile("a/b.json.bak")).toBe(false);
  });
});

describe("universalDesignName", () => {
  it("is the basename without .json", () => {
    expect(universalDesignName("/x/board.json")).toBe("board");
    expect(universalDesignName("/x/board.netlist.json")).toBe("board.netlist");
    expect(universalDesignName("/x/my.design.JSON")).toBe("my.design");
  });
});
...
describe("discoverUniversalDesigns", () => {
  it("lists valid netlists, lists shaped-but-broken ones with an error, skips the rest", async () => {
    ...
    expect(designs.map((d) => d.name)).toEqual(["demo-board.netlist", "pin-on-other-net"]);

    const demo = designs.find((d) => d.name === "demo-board.netlist")!;
  ...
  it("honours maxDepth", async () => {
    ...
    expect(designs.map((d) => d.name)).toEqual(["demo-board.netlist"]);
  });
  ...
  it("does not walk node_modules or dot-directories", async () => {
    ...
    try {
      ...
      await writeFile(path.join(root, "node_modules", "pkg", "a.json"), netlist);
      await writeFile(path.join(root, ".cache", "b.json"), netlist);
      await writeFile(path.join(root, "designs", "c.json"), netlist);
