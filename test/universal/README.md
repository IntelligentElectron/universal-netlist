...
| File | What it is |
| --- | --- |
| `demo-board.netlist.json` | A valid, hashed, UTC-dated netlist: named and unnamed pin entries, a member with several pins, a do-not-stuff part (`dns: true`), a test point, an unconnected pin (`""`), a single-pin net, and an auto-named multi-pin net. `run_erc` fires every rule it has on it. |
